...
class CMMDDataset(Dataset):
    def __init__(self, path, img_col, reshape_to, max_count=-1):
        self.path = path
        ...
        img_path_list = self._get_image_list_from_csv()
        if max_count > 0:
    ...
    def _get_image_list_from_csv(self):
        with open(self.path, "r") as f:
            reader = csv.DictReader(f)
            img_list = [row[f'{self.img_col}'] for row in reader]
        img_list.sort()
        return img_list
...
def compute_embeddings_for_dir(
    ...
    Args:
      ...
      max_count: Max number of images in the directory to use.s

    Returns:
      Computed embeddings of shape (num_images, embedding_dim).
    """
    dataset = CMMDDataset(path=log_file, img_col=img_dir, reshape_to=embedding_model.input_image_size, max_count=max_count)
    count = len(dataset)
