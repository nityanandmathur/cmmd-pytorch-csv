...
def compute_cmmd(log_file, ref_dir, eval_dir, ref_embed_file=None, batch_size=32, max_count=-1):
    ...
    if ref_embed_file is not None:
        ...
        ref_embs = io_util.compute_embeddings_for_dir(log_file, ref_dir, embedding_model, batch_size, max_count).astype(
            "float32"
        )
    eval_embs = io_util.compute_embeddings_for_dir(log_file, eval_dir, embedding_model, batch_size, max_count).astype("float32")
...
def main(argv):
    ...
    _, log_file, dir1, dir2 = argv
    print(
        "The CMMD value is: "
        f" {compute_cmmd(log_file, dir1, dir2, _REF_EMBED_FILE.value, _BATCH_SIZE.value, _MAX_COUNT.value):.3f}"
    )
